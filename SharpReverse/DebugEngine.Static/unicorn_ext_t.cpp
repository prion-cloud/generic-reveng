...
template <typename T>
uc_err uc_ext_mem_read(uc_engine* uc, const uint64_t address, T& t, const int offset)
{
...
template <typename T>
uc_err uc_ext_mem_write(uc_engine* uc, const uint64_t address, T t, const int offset)
{
    const auto size = sizeof(T);
