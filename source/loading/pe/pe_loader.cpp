...
namespace grev
{
    pe_loader::pe_loader(std::u8string const& data) :
        data_(data),
        header_(pe_header::inspect(data)) { }
    ...
    std::optional<std::uint32_t> pe_loader::entry_point_address() const
    {
...
static_assert(std::is_copy_constructible_v<grev::pe_loader>);
static_assert(std::is_nothrow_move_constructible_v<grev::pe_loader>);

static_assert(std::is_copy_assignable_v<grev::pe_loader>);
static_assert(std::is_nothrow_move_assignable_v<grev::pe_loader>);
