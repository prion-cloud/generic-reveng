...
{
    bool equal_to<grev::expression>::operator()(grev::expression const& expression_1, grev::expression const& expression_2) const
    {
        static constexpr std::hash<grev::expression> hash;
        return hash(expression_1) == hash(expression_2);
    }
    std::size_t hash<grev::expression>::operator()(grev::expression const& expression) const
    {
        return Z3_get_ast_hash(grev::expression::context(), expression);
    }
}
...
static_assert(std::is_destructible_v<grev::expression>);

static_assert(std::is_copy_constructible_v<grev::expression>);
static_assert(std::is_copy_assignable_v<grev::expression>);

static_assert(std::is_move_constructible_v<grev::expression>);
static_assert(std::is_move_assignable_v<grev::expression>);
